...
SESSION_TIMEOUT = require 'app/modules/users/const/session_timeout'
...
service =
	###
		...
		chat 							Chat 			Чат 															Да
	###
	newMessage: (users, data, needSendPushToUsers) -> @send users, 'notification/message', data, needSendPushToUsers

	###
		Уведомление об изменении прочитанности сообщения

		Название поля 		Тип поля 	Описание 													Обязательность
		chat 							Chat 			Чат 															Да
		message_id 				string 		Идентификатор сообщения 					Да
	###
	changeMessageRead: (users, data, needSendPushToUsers) -> @send users, 'notification/message/read', data, needSendPushToUsers

	###
		Уведомление об изменении структуры сообщений в чате

		Название поля 		Тип поля	Описание 												Обязательность
		chat				 			Chat 			Модель чата											Да
	###
	invalidateChat: (users, data, needSendPushToUsers) -> @send users, 'notification/chat/invalidate', data, needSendPushToUsers
	...
	###
		...
		message_id 				string 		Идентификатор сообщения 					Да
	###
	messageRead: (users, data, needSendPushToUsers) -> @send users, 'message/read', data, needSendPushToUsers

	###
		Уведомление об изменении статуса пользователя онлайн

		Название поля 		Тип поля 	Описание 													Обязательность
		user_id 					string 		Идентификатор пользователя 				Да
		online 						boolean 	Информация о том, 								Да
																находится ли пользователь сейчас
																онлайн
	###
	userOnline: (users, data, needSendPushToUsers) -> @send users, 'notification/user/online', data, needSendPushToUsers

	###
		Уведомление об изменении чата

		Название поля 		Тип поля 	Описание 													Обязательность
		chat 							Chat 			Модель чата								 				Да
	###
	changeChat: (users, data, needSendPushToUsers) -> @send users, 'notification/chat/change', data, needSendPushToUsers

	###
		Уведомление об удалении чата

		Название поля 		Тип поля 	Описание 													Обязательность
		chat_id 					string 		Идентификатор чата 								Да
	###
	deleteChat: (users, data, needSendPushToUsers) -> @send users, 'notification/chat/delete', data, needSendPushToUsers
	...
	send: (users = [], event, data, needSendPushToUsers = []) ->
		...
		Promise
			...
			.then (users) =>
				...
				users = _.filter users, (x) -> not x.is_blocked
	...
	hasSessionSocket: (session) ->
		@getRedisList REDIS_VARIABLE.SESSIONS_ROOMS
			.then ([range]) ->
	...
	pushRedisInRoom: (room, value) ->
		redisClient.multi()
			.rpush room, if _.isObject(value) then JSON.stringify(value) else value
			.execAsync()

	remRedisInRoom: (room, value, count = 1) ->
		redisClient.multi()
			.lrem room, count, if _.isObject(value) then JSON.stringify(value) else value
			.execAsync()
	...
	getRedisValue: (value) ->
		redisClient.multi()
			.get if _.isObject(value) then JSON.stringify(value) else value
			.execAsync()

	getRedisList: (list, start = 0, end = -1) ->
		redisClient.multi()
			.lrange list, start, end
			.execAsync()

	saddRedisMember: (key, value) ->
		redisClient.multi()
			.sadd key, value
			.execAsync()

	sscanRedisMember: (key, pattern) ->
		redisClient.multi()
			.sscan key, 0, 'MATCH', pattern
			.execAsync()
			.then ([[n, data]]) -> data

	sremRedisMember: (key, value) ->
		redisClient.multi()
			.srem key, value
			.execAsync()
	...
	_sendSocket: (users, sessions, event, dataPromise) ->
		...
		Promise
			.all _.map sessions, (x) ->
				x.getClientKey()
					.then (key) ->
						session: x
						key: key

			.then (sessions) -> Promise.all [sessions, dataPromise]
			.then ([sessions, data]) =>
				@emitLogger.log """\n
					event: #{event}
...
#	Promise.all [
#		service.clearRedisList REDIS_VARIABLE.SESSIONS_ROOMS
#		service.clearRedisList REDIS_VARIABLE.DISCONNECTS
#		service.delRedisKey REDIS_VARIABLE.LINKS
#		service.delRedisKey REDIS_VARIABLE.MESSAGES
#	]
